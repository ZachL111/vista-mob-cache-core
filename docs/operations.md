...
`vista-mob-cache-core` is centered on mobile workflows work in Go. The technical target is: Create a Go reference implementation for cache workflows, centered on incremental indexing, append-only fixtures, and checkpoint recovery checks.
